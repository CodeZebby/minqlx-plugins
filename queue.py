...
ROUND_BASED_TEAM_GAMETYPES = ("ca", "ad", "ft", "ift") # rr left out on purpose!
TEAM_BASED_GAMETYPES = ("ca", "ctf", "dom", "ft", "tdm", "ad", "1f", "har", 'rr')
NONTEAM_BASED_GAMETYPES = ("ffa", "race") # zebby: iffa and duel not supported?
...
class queue(minqlx.Plugin):
    ...
    def remFromQueue(self, player, update=True):
        ...
        if player in self._queue:
            # if self.is_endscreen:
                # self.test_logger.warning("queue.py: removing {} from queue".format(player))
                # self.msg("queue.py: removing {} from queue".format(player))
    ...
    def pushFromQueue(self, delay=0):
        ...
        @minqlx.next_frame
        def pushToTeam(amount, team, limit):
            ...
            if not self.is_endscreen:
                ...
                for count, player in enumerate(self._queue, start=1):
                    ...
                    if player in self.teams()['spectator'] and player.connection_state == 'active':
                        ...
                        # self.msg('queue.py: Calling remFromQueue 1 for {} - connection state: {}'.format(player.name, player.connection_state))
            ...
            if len(self._queue) > 1 and not self.is_endscreen:
                ...
                if self._queue[0] in spectators and self._queue[0].connection_state == 'active':
                    if self._queue[1] in spectators and self._queue[1].connection_state == 'active':
                        ...
                        # self.msg('queue.py: Calling remFromQueue 2 for {} - connection state: {}'.format(self._queue[1].name, self._queue[1].connection_state))
                        self.remFromQueue(self._queue[1])
                elif self._queue[0].connection_state not in ['connected', 'primed']:
                    # self.msg('queue.py: Calling remFromQueue 3 for {} - connection state: {}'.format(self._queue[0].name, self._queue[0].connection_state))
        ...
        def checkForPlace():
            ...
            support_red_rover = '0'
            try:
                support_red_rover = self.get_cvar("qlx_unevenTeamsSupportRedRover")
            except:
                pass
            ...
            if self.game.type_short in TEAM_BASED_GAMETYPES:
                ...
                players_in = red_amount + blue_amount
                limit = maxplayers - players_in
                if players_in >= maxplayers:
                    pass
                elif diff > 0 and not self.is_blue_locked:
                    pushToTeam(diff, "blue", limit)
                elif diff < 0 and not self.is_red_locked:
                    pushToTeam(-diff, "red", limit)
                elif diff == 0 and players_in < maxplayers:
                    ...
                    # elif self.game.state == 'warmup': # for the case if there is 1 player in queue
                        ...
                        or self.game.type_short not in ROUND_BASED_TEAM_GAMETYPES
                        or (self.game.type_short == 'rr' and support_red_rover != '1')
                    ):
                ...
                if free_amount < maxplayers:
                    limit = maxplayers - free_amount
                    pushToTeam(limit, "free", limit)
    ...
    def setAFK(self, player):
        ...
        if player in self.teams()['spectator'] and player not in self._afk:
            ...
            # self.msg('queue.py: Calling remFromQueue 4 for {} - connection state: {}'.format(player.name, player.connection_state))
            self.remFromQueue(player)
            return True
        return False
    ...
    def updTag(self, player):
        ...
        if player in self.players():
            ...
            if position > -1: # If the player is in the queue.
    ...
    def handle_player_disconnect(self, player, reason):
        ...
        # self.msg('queue.py: Calling remFromQueue 5 for {} - connection state: {}'.format(player.name, player.connection_state))
    ...
    def handle_team_switch(self, player, old_team, new_team):
        # self.msg('queue.py: team_switch: player:  {}'.format(player.name))
        # self.msg('queue.py: team_switch: old_team: {}'.format(old_team))
        # self.msg('queue.py: team_switch: new_team: {}'.format(new_team))

        if new_team != "spectator":
            # self.msg('queue.py: Calling remFromQueue 6 for {} - connection state: {}'.format(player.name, player.connection_state))
            self.remFromQueue(player)
            self.remAFK(player)
        else: # New team is 'spectator'
            self.updTag(player)
            self.pushFromQueue(0.5)
    ...
    def handle_team_switch_attempt(self, player, old_team, new_team):
        ...
        # self.msg('queue.py: team_switch_attempt: player:  {}'.format(player.name))
        # self.msg('queue.py: team_switch_attempt: old_team: {}'.format(old_team))
        # self.msg('queue.py: team_switch_attempt: new_team: {}'.format(new_team))

        if new_team != "spectator" and old_team == "spectator":
            teams = self.teams()
            maxplayers = self.get_maxplayers()
            if (
                len(teams["red"]) + len(teams["blue"]) >= maxplayers
                or len(teams["free"]) >= maxplayers
        ...
        def handler():
            if command == "team s":
                if player in self.teams()['spectator']:
                    # self.msg('queue.py: Calling remFromQueue 7 for {} - connection state: {}'.format(player.name, player.connection_state))
                    self.remFromQueue(player)
                    if player not in self._queue:
    ...
    def handle_new_game(self):
        self.handleQueueAtNewGame()

    @minqlx.thread
    def handleQueueAtNewGame(self):
        time.sleep(0.25)
